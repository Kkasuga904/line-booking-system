...
start "予約システム1" cmd /k "cd /d %~dp0 && set STORE_ID=account-1&& set STORE_NAME=予約システム1&& set PORT=3001&& set BASE_URL=http://localhost:3001&& set LINE_CHANNEL_ACCESS_TOKEN=%ACCOUNT1_TOKEN%&& set LINE_CHANNEL_SECRET=%ACCOUNT1_SECRET%&& set FEATURES=***REMOVED-ROTATE-CREDENTIAL***&& node store-instance-server.js"

timeout /t 3 /nobreak > nul

REM アカウント2起動
echo [2/2] 予約システム2 を起動中...
start "予約システム2" cmd /k "cd /d %~dp0 && set STORE_ID=account-2&& set STORE_NAME=予約システム2&& set PORT=3002&& set BASE_URL=http://localhost:3002&& set LINE_CHANNEL_ACCESS_TOKEN=%ACCOUNT2_TOKEN%&& set LINE_CHANNEL_SECRET=%ACCOUNT2_SECRET%&& set FEATURES=***REMOVED-ROTATE-CREDENTIAL***&& node store-instance-server.js"
