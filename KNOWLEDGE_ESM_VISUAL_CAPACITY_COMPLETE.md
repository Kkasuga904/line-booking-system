...
```javascript
// リクエスト
GET /api/capacity-status?store_id=***REMOVED-ROTATE-CREDENTIAL***&date=2025-08-30
...
async function updateTimeSlotVisuals(selectedDate) {
  const response = await fetch(
    `https://line-booking-api-116429620992.asia-northeast1.run.app/api/capacity-status?store_id=***REMOVED-ROTATE-CREDENTIAL***&date=${selectedDate}`
  );
...
### APIテスト
```bash
curl -X GET "https://line-booking-api-116429620992.asia-northeast1.run.app/api/capacity-status?store_id=***REMOVED-ROTATE-CREDENTIAL***&date=2025-08-30"
```
