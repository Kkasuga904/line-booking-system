...
module.exports = async (req, res) => {
    ...
    res.setHeader(
        ...
        'X-CSRF-Token, X-Requested-With, Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, X-Api-Version, X-Line-Signature'
    );
    
    // GETリクエスト（Verify用）
    if (req.method === 'GET') {
        return res.status(200).json({ status: 'OK', message: 'Webhook is ready' });
    }
...
async function handleEvent(event, client) {
    ...
    try {
        ...
        // プラン選択処理
        if (session && session.step === 'plan_selection') {
        ...
        // 連絡先入力処理
        if (session && session.step === 'contact_input') {
        ...
        // メール入力処理
        if (session && session.step === 'email_input') {
