...
```
予約の処理に失敗しました。
エラー: Store ID mismatch: frontend=default, server=***REMOVED-ROTATE-CREDENTIAL***
```
...
- **バックエンド** (`server.js`): `'***REMOVED-ROTATE-CREDENTIAL***'` を期待
...
// server.js (元のコード)
const storeId = process.env.STORE_ID || '***REMOVED-ROTATE-CREDENTIAL***';  // ← 異なるデフォルト値
```
...
const storeId = urlParams.get('store_id') || '***REMOVED-ROTATE-CREDENTIAL***';  // 統一
```

### 2. サーバー側の柔軟な対応
```javascript
// server.js (修正後)
// defaultと***REMOVED-ROTATE-CREDENTIAL***を同じものとして扱う
const normalizedFrontendStoreId = store_id === 'default' ? '***REMOVED-ROTATE-CREDENTIAL***' : store_id;
const normalizedServerStoreId = storeId === 'default' ? '***REMOVED-ROTATE-CREDENTIAL***' : storeId;
...
module.exports = {
    DEFAULT_STORE_ID: '***REMOVED-ROTATE-CREDENTIAL***',
    // その他の共通定数
};
```
...
- ❌ **悪い例**: 各ファイルで独立して定義
  ...
  // file2.js
  const storeId = '***REMOVED-ROTATE-CREDENTIAL***';
  ```
...
```javascript
// 許可されたStore IDのリスト
const VALID_STORE_IDS = ['***REMOVED-ROTATE-CREDENTIAL***', 'store-001', 'store-002'];
