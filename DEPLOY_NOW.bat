...
echo ***REMOVED-ROTATE-CREDENTIAL***| "%GCLOUD%" secrets create line-channel-secret-store-a --data-file=- --project=%PROJECT_ID% 2>nul
if %ERRORLEVEL% EQU 0 (
...
echo ***REMOVED-ROTATE-CREDENTIAL***| "%GCLOUD%" secrets create line-access-token-store-a --data-file=- --project=%PROJECT_ID% 2>nul
if %ERRORLEVEL% EQU 0 (
    echo Created successfully
) else (
    echo Already exists
...
"%GCLOUD%" run deploy %SERVICE_NAME% ^
  ...
  --set-env-vars "STORE_ID=store-a,NODE_ENV=production,SUPABASE_URL=***REMOVED-ROTATE-CREDENTIAL***,SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZhZW52enplZ3V2bGNvbnZycWdwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzUzNTIxMDksImV4cCI6MjA1MDkyODEwOX0.DhX6PDWkNjRydPDLRqKXk8Trh9EILrN7kLanFPLaYpk,LIFF_ID_STORE_A=***REMOVED-ROTATE-CREDENTIAL***" ^
