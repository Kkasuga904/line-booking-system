...
```javascript
// テンプレート一覧取得
GET /api/capacity-templates/templates?store_id=***REMOVED-ROTATE-CREDENTIAL***
...
```sql
-- 容量設定確認
SELECT * FROM capacity_templates WHERE store_id = '***REMOVED-ROTATE-CREDENTIAL***';

-- 適用ルール確認
SELECT * FROM capacity_rules WHERE store_id = '***REMOVED-ROTATE-CREDENTIAL***';
