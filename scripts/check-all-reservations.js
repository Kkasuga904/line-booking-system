...
async function checkAllReservations() {
  ...
  try {
    ...
    // ***REMOVED-ROTATE-CREDENTIAL***の詳細確認
    console.log('\n=== ***REMOVED-ROTATE-CREDENTIAL***詳細 ===');
    const defaultStoreRes = storeGroups['***REMOVED-ROTATE-CREDENTIAL***'] || [];
    if (defaultStoreRes.length === 0) {
      console.log('***REMOVED-ROTATE-CREDENTIAL***の予約はありません');
      
      // 環境変数値の確認
      console.log('\n環境変数STORE_ID:', process.env.STORE_ID || '未設定');
      console.log('デフォルト値: "***REMOVED-ROTATE-CREDENTIAL***"');
    } else {
      console.log(`***REMOVED-ROTATE-CREDENTIAL***の予約: ${defaultStoreRes.length}件`);
