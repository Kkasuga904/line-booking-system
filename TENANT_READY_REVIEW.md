...
2. URLパラメータ (store_id)
3. 環境変数 (STORE_ID)
4. デフォルト値 ('***REMOVED-ROTATE-CREDENTIAL***')
...
- **public/admin-full-featured.html**
  ...
  - ✅ すべての`***REMOVED-ROTATE-CREDENTIAL***`ハードコードを削除

## 3. レビュー観点チェック

### ✅ store依存値がコードに埋まっていないか
- `utils/store-config.js`で一元管理
- ハードコードされた`***REMOVED-ROTATE-CREDENTIAL***`を設定参照に置き換え
...
### ✅ 既存挙動に影響を与えていないか
- デフォルト値として`***REMOVED-ROTATE-CREDENTIAL***`を維持
